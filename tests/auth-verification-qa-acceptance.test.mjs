...
test('email verification QA acceptance is pinned to a fresh disposable production child and deletes it',()=>{
  ...
  assert.match(source,/Unexpected Phase 1 email-verification baseline/);
  ...
  assert.match(source,/waitVerificationDelivery\(workerBase,1\)/);
  assert.match(source,/waitVerificationDelivery\(workerBase,2\)/);
  assert.match(source,/\/send-verification-email/);
  assert.match(source,/AUTH_VERIFY_QA_RESEND/);
  assert.match(source,/assertRepeatedVerificationRejected\(deliveredLink\)/);
  assert.match(source,/AUTH_VERIFY_QA_REPEAT_LINK/);
  assert.match(source,/callbackURL:'https:\/\/packone\.pro\/\?auth=verify'/);
