...
test('email verification QA acceptance is pinned to a fresh disposable production child and deletes it',()=>{
  ...
  assert.match(source,/BRANCH='br-floral-truth-ayt6qqq9'/);
  ...
  assert.match(source,/PACK1_QA_AUTO_VERIFY_AFTER_SEND:'0'/);
  ...
  assert.match(source,/waitDeliveredVerificationLink\(beforeVerificationEmails\)/);
