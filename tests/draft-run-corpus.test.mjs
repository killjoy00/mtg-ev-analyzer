...
test('unanswered payloads contain neither the answer nor model rankings, support, or source identity',()=>{
  ...
  assert.deepEqual(Object.keys(visible).sort(),['candidates','pick_number','prior_picks','puzzle_id','set_id']);
});
test('seeded ten-round runs and both reroll orders obey exact early picks, buckets, source exclusions and budget-compatible replacement',()=>{
  for(let i=0;i<40;i++){
    ...
    run.forEach((source,round)=>{
      ...
      for(const order of [['set','pack'],['pack','set']]){
        ...
        for(const type of order){const p=selectDraftRunReroll(pool,current,{type,round,seed,excludedSources:seen});assert.ok(p);assert.ok(eligiblePickForRound(round,p.pick_number));assert.ok(!seen.includes(p.source_draft_hash));assert.equal(p.set_id===current.set_id,type==='pack');seen.push(p.source_draft_hash);current=p;}
      }
    });
  }
});
