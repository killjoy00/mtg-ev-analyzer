  ...
  for(let i=0;i<30;i++){
    ...
    for(let round=0;round<8;round++){
      ...
      for(let reroll=0;reroll<2;reroll++){
        ...
        assert.ok(replacement,`missing Cube replacement seed=${seed} round=${round} reroll=${reroll} source=${current.puzzle_id} pick=${current.pick_number} seen=${seen.length}`);assert.equal(replacement.set_id,environment);assert.ok(!seen.includes(replacement.source_draft_hash));
