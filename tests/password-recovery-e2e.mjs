...
for(const [name,type] of [['chromium',chromium],['webkit',webkit]]) {
  ...
  await page.goto(base+'/tests/password-recovery-harness.html');
  await page.locator('#account-signin').waitFor();
  assert.equal(await page.locator('body').getAttribute('data-harness-error'),null,name+' harness failed to render account UI');
  ...
  resetMode='policy';
  await page.goto(base+'/reset-password/?e2e=policy#token='+token);
  await page.locator('#reset-password-form').waitFor();
  assert.equal(page.url(),base+'/reset-password/','fragment token and harmless query state are scrubbed after full navigation');
