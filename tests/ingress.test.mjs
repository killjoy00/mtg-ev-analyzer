...
test('operations reject arbitrary commands and existing branch targets',()=>{
  for(const operation of ['idle','check-access','deploy-preview','disable-preview','probe-auth-webhook','stage-auth-webhook-probe','run-auth-webhook-probe','cleanup-auth-webhook-probe','deploy-authhook-qa','remove-authhook-qa'])assert.equal(parseRequest({operation,reason:'reviewed test'}),operation);
