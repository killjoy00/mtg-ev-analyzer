...
async function call(service,path,body,token,status=200,extra={}) {
  ...
  const r=await withPracticeAccess(request,query,r=>httpPrefix?fetch(r):service.fetch(r));
...
const practiceKey='practice-'+tag+'-retry';
let s=await call(runApi,'/v1/runs',{},guest.token,200,{headers:{'x-idempotency-key':practiceKey}});
const retriedPractice=await call(runApi,'/v1/runs',{},guest.token,200,{headers:{'x-idempotency-key':practiceKey}});
assert.equal(retriedPractice.id,s.id,'Exact practice creation retry must return the original run');
await call(runApi,'/v1/runs',{environment:'powered-cube'},guest.token,409,{headers:{'x-idempotency-key':practiceKey}});
assert.equal((await query('SELECT count(*) n FROM draft_run_sessions WHERE player_id=$1::uuid AND idempotency_key_hash IS NOT NULL',[guest.playerId])).rows[0].n,'1','Retry key must not create a second practice run');
