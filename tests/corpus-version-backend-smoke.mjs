...
assert.ok(!oldIds.includes(daily.current.puzzle_id));
assert.equal((await query('SELECT corpus_version FROM draft_run_sessions WHERE id=$1::uuid',[daily.id])).rows[0].corpus_version,DRAFT_RUN_CORPUS_VERSION);
assert.equal((await query("SELECT corpus_version FROM draft_run_schedules WHERE day=$1::date AND environment='mixed'",[gameDateKey()])).rows[0].corpus_version,DRAFT_RUN_CORPUS_VERSION);
console.log(JSON.stringify({corpusTransition:'ready',oldResume:true,oldGrading:true,oldChallenge:true,rerollsStayInVersion:true,newDailyUsesCurrent:true}));
