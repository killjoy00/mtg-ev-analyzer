...
test('scheduler release is manual-only and verifies a deployed reviewed main ancestor before enable',()=>{
  ...
  assert.match(flow,/git merge-base --is-ancestor "\$RELEASE_COMMIT" origin\/main/);
  assert.match(flow,/git cat-file -e "\$RELEASE_COMMIT\^\{commit\}"/);
  assert.match(flow,/grep -F "pack1-daily-primary"/);
  assert.match(flow,/grep -F "pack1-account-deletion-maintenance"/);
