...
const user=crypto.randomUUID(),other=crypto.randomUUID(),token=crypto.randomUUID(),hash='a'.repeat(64),secondHash='b'.repeat(64);
...
try {
  ...
  console.log('Patreon SQL lifecycle passed: exact tier, server classification, identity stability, upgrade/downgrade, unique account, stale snapshot, cancellation, expiry, disconnect, manual grant preservation.');
} finally {
