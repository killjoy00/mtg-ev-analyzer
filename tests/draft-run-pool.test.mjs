...
test('pool spans multiple bounded SQL responses without sampling',async()=>{
  const source=Array.from({length:10001},(_,i)=>({puzzle_id:String(i+1).padStart(8,'0'),pick_number:'2',candidate_count:'14',consensus_top_gap:'0.1',support_entropy:'0.7',difficulty_version:'support-ratio-v1',rating:'70',top_two_ratio:'0.7',target_support_ratio:'1'}));
  let calls=0;
...
test('nonadvancing cursor fails instead of looping',async()=>{
  await assert.rejects(loadVerifiedPool(async()=>({rows:[{puzzle_id:'a',difficulty_version:'support-ratio-v1',top_two_ratio:0.7}]}),'v6',1),/did not advance/);
});
