...
const options={modules:true,scriptPath:process.argv[2],compatibilityDate:'2026-09-01',compatibilityFlags:['nodejs_compat'],
  durableObjects:{NETWORK_QUOTA:{className:'NetworkQuota',useSQLite:true}},durableObjectsPersist:persist};
let mf;
try {
  mf=new Miniflare(options);
  let ns=await mf.getDurableObjectNamespace('NETWORK_QUOTA');
  ...
  await mf.dispose();mf=new Miniflare(options);ns=await mf.getDurableObjectNamespace('NETWORK_QUOTA');
