...
try{
  ...
  const zoomControl=page.locator('.run-zoom').first(),zoomTarget=await zoomControl.boundingBox(),zoomImage=await page.locator('.run-card-select img').first().boundingBox();assert.ok(zoomTarget&&zoomTarget.height>=43.5&&zoomTarget.width>=43.5,'Enlarge control meets the 44px target');assert.equal((await zoomControl.innerText()).trim(),'','card zoom uses an icon instead of repeated visible text');assert.match(await zoomControl.getAttribute('aria-label'),/^Enlarge /);assert.ok(zoomImage&&zoomTarget.x>=zoomImage.x+zoomImage.width-zoomTarget.width-12&&zoomTarget.y<zoomImage.y+52,'zoom control overlays the card corner');
