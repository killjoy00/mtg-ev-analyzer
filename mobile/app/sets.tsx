...
function SetCard({ entry, special = false }: { entry: SetCatalogEntry; special?: boolean }) {
  return (
    <View style={styles.card}>
      ...
      <Pressable
        ...
        onPress={() => special
          ? router.push({ pathname: '/draft-run', params: { environment: 'powered-cube' } })
          : router.push({ pathname: '/draft-run', params: { environment: 'mixed' } })}
        style={styles.cardAction}
      >
        <Text style={styles.cardActionText}>{special ? 'Play Cube Daily →' : 'Play Draft Run Daily →'}</Text>
      </Pressable>
    </View>
  );
}
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.page}>
        ...
        <View style={styles.section}>
          ...
          {standard.length ? standard.map((entry) => <SetCard key={entry.set_id} entry={entry} />) : (
        ...
        {special.length ? (
          <View style={styles.section}>
            ...
            {special.map((entry) => <SetCard key={entry.set_id} entry={entry} special />)}
          </View>
        ) : null}
      </ScrollView>
    </SafeAreaView>
