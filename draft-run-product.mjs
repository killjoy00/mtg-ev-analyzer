...
function render() {
  ...
  app().innerHTML=`<section class="draft-run-page"><header class="run-heading"><div><p class="eyebrow">${run.day?'Daily ':''}${title()} · ${run.day||'Practice'}</p><h1>${esc(setName(p.set_id))} <span>Pack 1 · Pick ${p.pick_number}${p.difficulty?` · Difficulty: ${esc(p.difficulty.band[0].toUpperCase()+p.difficulty.band.slice(1))} (${p.difficulty.rating}/100)`:""}</span></h1></div><a class="text-button" href="./">Leave run</a></header>${steps()}
