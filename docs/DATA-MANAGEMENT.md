...
Traditional archives remain offline build inputs, but reviewed Traditional-derived playable components are now published additively. The current regular component is `traditional-premier-v4-phase2-v1`; Powered Cube uses `traditional-cube-p2p7-v4-v1` for P1P2-P1P7 only. Both are graded by the Premier-trained `strong-player-colour-stage-v4` model on parent `elite-trophy-colour-stage-v8`; Traditional rows do not train or calibrate that model. Twenty-one regular v4 components are Live, SIR is Candidate under its Candidate parent, and HBG/HOB/KTK/PIO remain blocked. Current serving can mix Live Traditional-derived and Premier-derived puzzles within the same eligible environment; historical v3 components remain readable. See [Traditional v4 release closeout](reports/TRADITIONAL-V4-RELEASE-2026-09-21.md).
