...
GitHub Actions performs the entire backfill without an AI session: discover all archives, import, validate, load development, complete mixed/Cube practice smoke tests, load production, and test production. The job starts when importer changes reach main, or through **Run workflow**. It saves archive checkpoints in Actions cache and the compact results as 90-day artifacts; database rows and per-set completion manifests persist in Neon. Failed validation blocks later steps, and idempotent inserts make rerunning safe. Repository owners can follow the job summary and GitHub's normal failure notifications. No model polling is needed.

Actions does not receive a database password. `/v1/trophy-import` accepts only a GitHub-signed, short-lived OIDC identity with the exact repository/owner IDs, main branch, import workflow path, allowed event, and dedicated audience. Pull requests and other workflows are rejected. The endpoint exposes bounded, validated, immutable trophy insertion and per-set status/finalization; it exposes no arbitrary SQL. Production loading follows the development gameplay gate. A new environment still requires explicit catalog registration, and source conflicts still stop rather than silently replacing published puzzles.
