...
## Track two: the grading study

`scripts/grading_study.py` covers the second track. It samples decisions from the
served corpus, writes a task file with every model number stripped out, and
compares the ratings that come back against what Pack One awards.

```bash
python3 scripts/grading_study.py sample --size 40 --targets 4 \
  --task grading-task.json --key grading-key.json

python3 scripts/grading_study.py compare --ratings ratings.json --key grading-key.json
```

The sample balances over how far the trophy pick sits below the pack leader
(leader / close / moderate / severe) and over pick depth, deliberately
over-sampling severe disagreements: they are rare in the corpus and they are
exactly the cases the curve is judged on. Each decision asks for ratings on the
pack leader, the trophy pick, and alternatives reaching down the support range.

Blinding is enforced, not merely intended: the task file carries no support
value, no ratio and no marker for which card led or which was taken, and the
cards to rate are hash-ordered rather than support-ordered. A test asserts it.

The headline output is the exponent human judgement implies in

    score = 95 x (selected support / leader support) ** exponent

Production is 1.0 today. Making the model's probabilities honest is equivalent
to roughly 2.0, because normalising cancels in the ratio. If raters imply 1.0 the
current curve is right and honest probabilities need compensating in the formula;
2.0 says the harsher curve is right; in between says how far to go. The fit
carries a bootstrap interval clustered over decisions, and a test confirms the
fitter recovers a known exponent from ratings generated by it.

It also reports what raters give the trophy pick when it is *not* the model
leader. Draft Run awards that card 100 automatically; whether raters agree is
testable and has never been tested.

## What honest probabilities would actually touch

Sharpening the model is equivalent to an exponent on the ratio, so it does not
need the corpus regenerated - `gradeDraftRunPick` is the single place that turns
support into a score, and it is linear today (`SUPPORT_EXPONENT = 0.75` lives in
the legacy `scoring.mjs` / `worker/core.mjs` path, not in Draft Run). Three
consequences are worth stating before anyone ships it:

* `app.js` shows raw support percentages next to the score, so those have to move
  with the curve or the two stop agreeing.
* `draft_run_verified_puzzle_ratings` stores a difficulty rating built from the
  same ratio. Leaving it alone keeps bands stable and keeps a scoring change
  interpretable, but difficulty and scoring would then read the supports
  differently.
* The leaderboard query in `worker/draft-run-function.mjs` aggregates `scores`
  without filtering on `scoring_version`, so a curve change mixes old generous
