...
The harness covers the first track. `scripts/grading_curve.py` covers the
second, and does it without anyone's judgement: a scoring curve can be tested
against whether it separates strong drafters from ordinary ones, and against
how early the drafting population actually takes a card.
