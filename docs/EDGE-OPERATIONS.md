...
Run [35288291535](https://github.com/killjoy00/mtg-ev-analyzer/actions/runs/35288291535) deployed and verified all 14 deny-only utility endpoints, all three guarded APIs and the Cloudflare custom domain. TLS, private access and direct-origin rejection passed. Full corpus health then returned 503: development still held 33 environments with zero unrated puzzles, while the reviewed catalog contains 32. Its temporary branch was removed. The preview workflow now applies the existing reviewed `0016` retirement migration in a transaction, only after proving it created the disposable branch, and runs the real corpus health handler before function deployment. This can delete retired data only in that disposable copy. It does not apply the migration to development or production, and it does not relax health requirements.
...
2. Grant **Editor** at the **Workers product scope** for the account containing `packone.pro`. If the dashboard shows the legacy custom-token permission table instead, the production configuration that passed the live telemetry query uses **Account → Workers Observability → Edit** together with **Workers Scripts → Edit**. Cloudflare's telemetry API reference may describe the writable observability capability as `Workers Observability Write`; do not look for a separate `Workers Observability Telemetry` row unless the dashboard actually presents one.
