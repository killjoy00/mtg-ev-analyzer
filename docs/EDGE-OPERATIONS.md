...
- Each deployment creates a new branch from development, with a 24-hour expiry and 300-second compute suspend. It deploys the exact main revision and checks schema readiness. Neon inherits function deployments when branching: the controller validates the inventory, removes every inherited function copy on its own newly created branch, verifies an empty inventory, then deploys only the three guarded APIs. Existing branches are refused. The workflow removes its own newly created branch on failure; expiry provides a fallback and removes successful previews after a day.
