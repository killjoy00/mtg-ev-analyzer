...
1. Correct and measure the production stage-reference isolation finding in [issue #164](https://github.com/killjoy00/mtg-ev-analyzer/issues/164) before the next versioned model release. Preserve current historical results.
2. Observe the next genuinely new archive through the automated Candidate pipeline. Keep explicit publication and existing quality gates; SIR/HBG/PIO remain Candidate and STX Retired.
3. Validate physical-device authentication, native sharing, and assistive technology with real devices. Browser viewport tests do not establish those results.
