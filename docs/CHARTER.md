...
States are Candidate, Live, Paused and Retired. Automated discovery/import creates non-serving Candidate records with visible readiness gates. A Candidate is ready for publication only after objective gates pass. Authenticated admins explicitly publish Live; every transition records actor, timestamp, old/new state and optional reason. Activation requires fresh passing checks. Publication never follows solely from archive availability. [Operations and thresholds](CORPUS-OPERATIONS.md).
