...
The [private gateway preview](EDGE-OPERATIONS.md) adds an opt-in origin guard and tests durable network quotas without changing the current public deployment. Existing direct-URL deployment, import and health workflows must be migrated before enabling that guard on production. The preview controller cannot select either existing Neon branch as its deployment target.
