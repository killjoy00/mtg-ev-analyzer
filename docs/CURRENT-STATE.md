...
Updated 2026-09-24. Implementation, backend deployment, database publication, and provider activation are separate. Older reviews are historical evidence.

**Pack One launch measurement release:** PR #505 merged to `main` as `572f36638a780f73a41a548eb07e3101ea2efed5`. GitHub Pages deployment 36091056841 succeeded, and the exact merge commit passed post-merge test 36091057812, E2E 36091057794, and production smoke 36091057833. The browser-side acquisition capture, social metadata, and Daily reset cue are therefore live. The backend-dependent pieces from #505 — the admin Daily habit metrics and the session-derived `daily_streak` field — are merged but **not yet promoted to production Functions**. `.github/workflows/deploy-functions.yml` is a manual development-then-production promotion, and no post-merge #505 function deployment has run yet. No schema migration is required for this release.
...
| Production functions | Development and production Functions now serve reviewed season-hardening application revision `9763e7caf87f20ca4bc77d82b8513a3d365db5c9` from PR #476. Migration 0038 passed development run 36020410138 and production run 36022058215; the same exact application revision passed development deploy/acceptance 36021141621 and production deploy/acceptance 36023616589. Production acceptance verified schema prerequisites, required the identical revision already accepted in development, deployed checked bundles, waited for stable release markers, completed all three unranked Daily flows, and queried the current-season leaderboard for mixed, Powered Cube and Latest Set. Profile season enrichment now fails open and `pack1growth` no longer creates immutable Daily schedules. |
| Schema | Reviewed migrations through 0038 are applied to development and production. Migration 0037 adds durable `draft_run_seasons`, a single-current-season constraint, and serialized reconciliation from immutable Latest Set Daily schedules. Migration 0038 adds a durable Latest Set reconciliation watermark so settled schedule history is never revalidated against mutable policy, while retaining the established season rows as product history. Migration 0035 moves the current retention/Daily calendar views to `America/Los_Angeles`; migration 0036 is the bounded legacy self-share cleanup. Historical destructive migration 0016 was not replayed. |
...
- Pack One launch attribution and habit measurement merged through PR #505 as `572f36638a780f73a41a548eb07e3101ea2efed5`. Final PR gates passed test 36090708568, E2E 36090708576, and isolated Neon backend schema/integration gate 36090708642. After merge, Pages deployment 36091056841, test 36091057812, E2E 36091057794, and production smoke 36091057833 all passed. This proves the browser release and the existing production contract; it does not substitute for the manual Functions promotion required to activate #505's new habit-report query and `daily_streak` response in production.
