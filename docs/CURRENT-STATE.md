...
| Production functions | Current production functions serve the reviewed scheduler-aware revision `4029202c96bebe893419efa91dd0eeea3407d285` from PR #423. It passed development deploy 35924506293 and production deploy 35925098968 with exact-revision, coverage and all-three-Daily acceptance. This revision includes the previously verified passwordless account-deletion email flow and Pacific Daily generation/trigger ingress. |
| Schema | Reviewed additive migrations through 0035 are applied to development and production. Migration 0035 moves the current retention/Daily calendar views to `America/Los_Angeles`; raw historical rows are unchanged. Earlier additions include owned Pack One username uniqueness, the v4 Traditional component guards, and `account_deletion_verifications` for passwordless deletion proof. Historical destructive migration 0016 was not replayed. |
