...
- Established account-linked player tokens qualify for Daily ranking without a redundant active Auth header. A completed guest fixed Daily can be promoted by an explicit sign-in/link on the same Eastern game date when the run belongs to the resolved player, is complete, is still unranked, and no score already exists for that player/date/environment. Successful validation inserts the one Daily score and sets `leaderboard_eligible`; fixed schedules, zero rerolls, account/day/environment uniqueness, merge conflict handling, and score uniqueness preserve first-attempt behavior. The first pack displays ranked identity or a guest warning.
