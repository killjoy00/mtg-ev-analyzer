...
| --- | --- |
| Browser | GitHub Pages serves `main`. Pack One launch engineering merged in PR #505 as `572f36638a780f73a41a548eb07e3101ea2efed5`; Pages deployment 36091056841 passed, so the refreshed evergreen metadata/social preview tags, UTM capture/removal client, and next-Daily countdown UI are published. Production smoke 36091057833 also passed against the live frontend and existing production backend. The September 24 graphical and user-journey refresh remains live through PRs #486-#490: focused three-Daily home, dedicated Practice hub, beginner-first How To Play entry, Learn hub, and Pack One Elite landing/activation flow. Unfinished Daily home no longer duplicates Practice or Elite options. On narrow signed-in screens the redundant Daily Run nav item is omitted while the Pack One brand still returns to Daily home. The live leaderboard periods are Today, This week, This season and All time; old `month` board links canonicalize to the current season. Passwordless accounts retain the permanent-confirmation, send/resend-code, and 8-digit verify-and-delete flow; password accounts keep the existing current-password form. |
| Production functions | Production Functions still serve reviewed season-hardening application revision `9763e7caf87f20ca4bc77d82b8513a3d365db5c9` from PR #476. PR #505 adds backend support for acquisition event properties, session-based Daily streaks, and launch habit/cohort reporting, but exact merged revision `572f36638a780f73a41a548eb07e3101ea2efed5` has not yet been promoted through the required development-then-production Functions workflow. Until that promotion completes, campaign/referrer properties may be dropped server-side, the result/home cue cannot show the new session streak, and the admin launch-cohort tables remain unavailable even though the browser code is live. The prior season-hardening production acceptance remains 36023616589. |
...
- Pack One launch engineering merged September 24 in PR #505 as `572f36638a780f73a41a548eb07e3101ea2efed5` after final PR gates passed: test 36090708568, E2E 36090708576, and isolated Neon backend gate 36090708642. GitHub Pages deployment 36091056841 and live production-contract smoke 36091057833 passed. This release adds sanitized first-touch UTM/referrer attribution, Daily-only next-day countdown/streak cues, session-based launch retention metrics, evergreen social metadata, and privacy disclosure. At this snapshot the browser half is live; the matching Functions revision still awaits the normal development then production promotion, so backend-dependent launch measurement/streak fields are not yet live.
