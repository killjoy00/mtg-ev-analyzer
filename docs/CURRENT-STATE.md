...
| Production functions | Current production functions serve reviewed season revision `fe9d666a094c0d18a13899a089a3ed49de67901a` from PR #460. It passed development deploy/acceptance 36001721901 and production deploy/acceptance 36004516512. Production acceptance required the identical revision already proven in development, verified schema prerequisites, completed all three unranked Daily flows, and required mixed, Powered Cube and Latest Set leaderboards to resolve the same current season and start. The revision retains the shared-run self-challenge prevention from `0649b3897a12e0d5ae98f3ecd693fccd768c0788`. |
| Schema | Reviewed migrations through 0037 are applied to development and production. Migration 0037 adds durable `draft_run_seasons`, a single-current-season constraint, and serialized reconciliation from immutable Latest Set Daily schedules while preserving established season metadata independently of mutable corpus policy. Migration 0035 moves the current retention/Daily calendar views to `America/Los_Angeles`; migration 0036 is the bounded legacy self-share cleanup. Historical destructive migration 0016 was not replayed. |
| Competitive seasons | The inaugural persisted season is `hob` / **The Hobbit**, release date 2026-08-14, with season start 2026-09-16 and first immutable Latest Set schedule on 2026-09-19. Temporary A → B → A set fallback does not roll the season backward. Current-season standings use the same canonical resolver across mixed, Powered Cube and Latest Set and appear on My Pack One/public profiles when the player has season results. |
| Recurring maintenance | Production Neon Function Triggers are enabled through PR #434 / scheduler release 35926957155: Daily primary at 00:07 Pacific, idempotent Daily retry at 00:37 Pacific, and bounded account-deletion maintenance every ten minutes. GitHub Daily generation and account-deletion maintenance are manual-only recovery/verification; player-triggered Daily creation remains the Daily fallback. |
