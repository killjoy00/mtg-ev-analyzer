...
| --- | --- |
| Browser | GitHub Pages serves `main`. PR406 browser changes merged as `ff2babe58f9d6c4c8d06879851aea9050ea5f7a6`; Pages deployment 35903733507 and post-merge E2E 35903734335 passed. The release-request-only follow-up `b65822a52e3df82ac8f14dffce87d6caa78a223b` did not change browser runtime. Passwordless accounts now receive the permanent-confirmation, send/resend-code, and 8-digit verify-and-delete flow; password accounts keep the existing current-password form. |
| Production functions | Current production functions serve reviewed revision `0649b3897a12e0d5ae98f3ecd693fccd768c0788` from PR #446. It passed development deploy/acceptance 35952934212 and production deploy/acceptance 35953672931. This revision retains the scheduler-aware runtime already verified in the preceding release and adds authoritative shared-run ownership: a creator reopening their own practice share recovers the original result rather than creating a self-challenge. |
| Schema | Reviewed migrations through 0036 are applied to development and production. Migration 0035 moves the current retention/Daily calendar views to `America/Los_Angeles`; migration 0036 is a bounded data repair that removes legacy same-player shared-run challenge artifacts after the prevention fix is live. Earlier additions include owned Pack One username uniqueness, the v4 Traditional component guards, and `account_deletion_verifications` for passwordless deletion proof. Historical destructive migration 0016 was not replayed. |
