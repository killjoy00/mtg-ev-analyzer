...
async function start(request) {
  ...
  if(rawIdempotencyKey&&!/^[A-Za-z0-9_-]{16,128}$/.test(rawIdempotencyKey))fail('Invalid idempotency key.');
  const idempotencyKeyHash=!daily&&rawIdempotencyKey?digest(rawIdempotencyKey):null;
  const idempotencyFingerprint=idempotencyKeyHash?digest(JSON.stringify({environment,setIds,challenge:source?.id||null})):null;
  if(idempotencyKeyHash) {
    const retry=await query('SELECT * FROM draft_run_sessions WHERE player_id=$1::uuid AND day IS NULL AND idempotency_key_hash=$2 LIMIT 1',[owner,idempotencyKeyHash]);
    if(retry.rows[0]) {
      if(retry.rows[0].idempotency_fingerprint!==idempotencyFingerprint)fail('That retry key was already used for a different run.',409);
      return json(await responseFor(decode(retry.rows[0])));
    }
  }
  ...
  let seed=day ? `daily:${environment}:${day}:${DRAFT_RUN_CORPUS_VERSION}:${DRAFT_RUN_SELECTION_VERSION}` : idempotencyKeyHash ? `practice:${idempotencyKeyHash}:${DRAFT_RUN_CORPUS_VERSION}:${DRAFT_RUN_SELECTION_VERSION}` : crypto.randomUUID();
  ...
  const inserted=await query(`INSERT INTO draft_run_sessions(player_id,day,seed,corpus_version,scoring_version,puzzle_ids,seen_sources,challenge_id,environment,rerolls,difficulty_version,difficulty_anchors,selection_version,measurement_qa,daily_featured_sets,daily_account_id,leaderboard_eligible,custom_set_ids,serving_policy_version,idempotency_key_hash,idempotency_fingerprint)
    ...
    ON CONFLICT DO NOTHING RETURNING *`,[owner,day,seed,corpusVersion,scoringVersion,JSON.stringify(ids),JSON.stringify(sources),source?.id||null,environment,JSON.stringify(rerolls),difficultyVersion,JSON.stringify(anchors),selectionVersion,body.qa===true,JSON.stringify(featuredSets),day?account?.auth_user_id||null:null,Boolean(day&&account),JSON.stringify(setIds),servingPolicy,idempotencyKeyHash,idempotencyFingerprint]);
  ...
  if(!s && idempotencyKeyHash) {
    s=(await query('SELECT * FROM draft_run_sessions WHERE player_id=$1::uuid AND day IS NULL AND idempotency_key_hash=$2 LIMIT 1',[owner,idempotencyKeyHash])).rows[0];
    if(s&&s.idempotency_fingerprint!==idempotencyFingerprint)fail('That retry key was already used for a different run.',409);
  }
