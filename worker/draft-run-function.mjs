...
import {
  ...
  selectDraftRun, selectDraftRunReroll, publicDraftRunPuzzle, validateDraftRunPuzzle,
} from '../draft-run.mjs';
...
async function share(id) {
  ...
  const r=await query('SELECT id,display_name,score,puzzle_ids FROM draft_run_shares WHERE id=$1',[id]);
...
async function persistResult(s) {
  ...
  await query(`WITH ranked AS (
    ...
    SELECT $1::uuid,$2::date,'mixed','draft_run',$3::int,$4,$5::jsonb,$6::jsonb,true WHERE $2::date IS NOT NULL
    ...
    VALUES($1::uuid,'mixed','draft_run',$3::int,$4,$7,$2::date IS NOT NULL,$8,$9,$10::int,$11,$12)
  ...
  [s.player_id,s.day,score,grade,JSON.stringify(s.answers.map(a=>a.selectedId)),JSON.stringify({run:s.id,scoring_version:s.scoring_version,historical_matches:s.answers.filter(a=>a.historicalMatch).length}),s.seed,s.challenge_id,exact?other.display_name:null,exact?other.score:null,outcome,`draft-run:${s.id}`,JSON.stringify(sets),JSON.stringify({mode:'draft_run',daily:Boolean(s.day),score,run_id:s.id,challenge:Boolean(other),outcome}),JSON.stringify(['game_completed',...(s.day?['daily_completed']:[]),...(exact?['challenge_complete']:[])])]);
}
...
async function responseFor(s) {
  ...
  let standing=null;
  if(complete && s.day) {
    const r=await query(`SELECT count(*) total,1+count(*) FILTER(WHERE score>$2::int) rank,count(*) FILTER(WHERE score>=$2::int) through_ties FROM scores WHERE challenge_date=$1::date AND mode='draft_run' AND set_id='mixed'`,[s.day,s.score]);
  ...
  return {id:s.id,day:s.day,revision:s.revision,round:s.answers.length+1,complete,score:s.score,answers:s.answers,rerolls:s.rerolls,current,comparison,standing,scoring_version:s.scoring_version};
}
...
async function start(request) {
  ...
  if(day) {
    const old=await query('SELECT * FROM draft_run_sessions WHERE player_id=$1::uuid AND day=$2::date',[owner,day]);
    if(old.rows[0]) return json(await responseFor(decode(old.rows[0])));
  }
  const choices=await pool();
  ...
  const seed=day ? `daily:${day}:${DRAFT_RUN_CORPUS_VERSION}` : crypto.randomUUID();
  let ids;
  if(source) ids=source.puzzle_ids;
  else if(day) {
    const plan=selectDraftRun(choices,seed).map(p=>p.puzzle_id);
    await query('INSERT INTO draft_run_schedules(day,corpus_version,puzzle_ids) VALUES($1::date,$2,$3::jsonb) ON CONFLICT(day) DO NOTHING',[day,DRAFT_RUN_CORPUS_VERSION,JSON.stringify(plan)]);
    const r=await query('SELECT puzzle_ids FROM draft_run_schedules WHERE day=$1::date',[day]);
    ids=parse(r.rows[0].puzzle_ids);
  } else ids=selectDraftRun(choices,seed).map(p=>p.puzzle_id);
  ...
  const inserted=await query(`INSERT INTO draft_run_sessions(player_id,day,seed,corpus_version,scoring_version,puzzle_ids,seen_sources,challenge_id)
    VALUES($1::uuid,$2::date,$3,$4,$5,$6::jsonb,$7::jsonb,$8) ON CONFLICT DO NOTHING RETURNING *`,[owner,day,seed,DRAFT_RUN_CORPUS_VERSION,DRAFT_RUN_SCORING_VERSION,JSON.stringify(ids),JSON.stringify(sources),source?.id||null]);
  let s=inserted.rows[0];
  if(!s && day) s=(await query('SELECT * FROM draft_run_sessions WHERE player_id=$1::uuid AND day=$2::date',[owner,day])).rows[0];
  if(!s) fail('Could not start your run. Please retry.',409);
  if(inserted.rows.length) await query('INSERT INTO analytics_events(player_id,event_name,event_props) VALUES($1::uuid,$2,$3::jsonb)',[owner,day?'daily_started':'game_started',JSON.stringify({mode:'draft_run',daily,challenge:Boolean(source),run_id:s.id})]);
...
async function change(request,id,action) {
  ...
  if(action==='pick') {
    ...
    if(s.rerolls[type]!==1) fail('That reroll has already been used.',409);
    const choices=await pool(),current=choices.find(p=>p.puzzle_id===body.puzzleId);
    if(!current) fail('This puzzle is unavailable.',503);
    const replacement=selectDraftRunReroll(choices,current,{type,round,seed:s.seed,excludedSources:s.seen_sources});
...
async function route(request) {
  ...
  if(request.method==='GET'&&path==='/health') { const p=await pool();return json({ok:true,service:'draft-run',scoring_version:DRAFT_RUN_SCORING_VERSION,corpus_version:DRAFT_RUN_CORPUS_VERSION,puzzles:p.length,sets:new Set(p.map(p=>p.set_id)).size}); }
  ...
  if(request.method==='GET'&&shared) { const s=await share(shared[1]);return json({id:s.id,name:s.display_name,score:s.score}); }
