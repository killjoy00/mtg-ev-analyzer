...
export async function insertTrophyBatch(query,puzzles,{componentVersion=null}={}) {
  ...
  for(const sid of dynamic){const r=await query("SELECT v.set_id FROM corpus_set_versions v JOIN corpus_sources s ON s.set_id=v.set_id AND s.event_type='PremierDraft' LEFT JOIN draft_run_environment_policy p ON p.set_id=v.set_id WHERE v.set_id=$1 AND v.corpus_version=$2 AND (p.status='Candidate' OR (p.set_id IS NULL AND v.manifest->>'discovered'='true' AND s.archive_available AND s.import_status IN ('building','validating')))",[sid,VERSION]);if(r.rows.length)registered.add(sid);}
