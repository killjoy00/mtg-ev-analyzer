...
export async function insertTrophyBatch(query,puzzles) {
  ...
  for(const sid of dynamic){const r=await query("SELECT p.set_id FROM draft_run_environment_policy p JOIN corpus_set_versions v USING(set_id) WHERE p.set_id=$1 AND p.status='Candidate' AND p.source_event_type='PremierDraft' AND v.corpus_version=$2",[sid,VERSION]);if(r.rows.length)registered.add(sid);}
  const batch=puzzles.map(p=>{
    if((!allowed.has(p.set_id)&&!registered.has(p.set_id))||!validateDraftRunPuzzle(p)||!['puzzle_id','source_draft_hash','source_fingerprint'].every(k=>new RegExp(k==='source_fingerprint'?'^[a-f0-9]{64}$':'^[a-f0-9]{32}$').test(p[k]))||[...p.candidates,...p.prior_picks].some(c=>!c.image_url?.startsWith('https://')))throw error('Invalid verified puzzle');
