...
export const DELETION_MAINTENANCE_AUDIENCE='pack-one-account-deletion-maintenance';
export const DELETION_MAINTENANCE_WORKFLOW=`${REPO}/.github/workflows/account-deletion-maintenance.yml@refs/heads/main`;
...
const denied=()=>{throw Object.assign(Error('Maintenance identity denied'),{status:403});};

export async function verifyDeletionMaintenanceToken(token,getKeys=keys,now=Math.floor(Date.now()/1000)) {
  ...
  const checks={
    issuer:c.iss===ISSUER,
    audience:c.aud===DELETION_MAINTENANCE_AUDIENCE,
    ...
    workflow:c.workflow_ref===DELETION_MAINTENANCE_WORKFLOW,
    reusableWorkflow:!c.job_workflow_ref||c.job_workflow_ref===DELETION_MAINTENANCE_WORKFLOW,
  ...
  if(rejected.length){console.warn('Deletion maintenance identity rejected',rejected.join(','));denied();}
