...
export async function linkAccount(authSessionToken) {
...
export async function getAuthSession() {
  try { return await authRequest('/get-session'); } catch { return null; }
}
export async function signUpAccount({ name, email, password }) {
  return authRequest('/sign-up/email', { method:'POST', body:{ name, email, password } });
}
export async function signInAccount({ email, password }) {
  return authRequest('/sign-in/email', { method:'POST', body:{ email, password, rememberMe:true } });
}
export async function signOutAccount() {
  try { return await authRequest('/sign-out', { method:'POST', body:{} }); } catch { return null; }
}
