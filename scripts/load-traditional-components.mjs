...
export function sourceQuality(report,sid,{puzzles,usable,metadataComplete,storedMetadataComplete=metadataComplete,imagesComplete},evidence=null) {
 ...
 return {ready:Object.values(gates).every(Boolean),gates,usable_puzzles:usable,puzzles,
  ...
  parity_picks:parity,premier_source_audit:s.premier_source_audit};
}
export async function validateComponents(root) {
 ...
 if(phase2) {
  ...
  if(!sameSet([...passing,...failing],expectedSets)||new Set([...passing,...failing]).size!==expectedSets.length)throw Error('Incorrect Phase 2 environment scope');
 ...
 for(const file of all.filter(f=>path.basename(f)==='manifest.json')) {
  const manifest=JSON.parse(fs.readFileSync(file)),sid=manifest.id,dir=path.dirname(file),reportSet=report.sets[sid];
  ...
  if(!s.traditional_cohort||ledger.size!==s.traditional_cohort.trophy_outcomes['3-0']||qualified!==s.traditional_cohort.qualified_trophies||included*width!==manifest.puzzles)throw Error('Traditional trophy accounting mismatch');
...
export async function importComponents(query,prepared) {
 for(const s of prepared.filter(s=>s.health.ready)) {
  ...
  const signature=s.manifest.frozen_input_signature||s.manifest.model_input_signature;
  ...
  if(!expectedModel||base?.model_version!==expectedModel||base.input_signature!==signature)throw Error('Parent model evidence differs from the validated artifact');
 ...
 for(const s of prepared) {
  ...
  await query("INSERT INTO corpus_components(set_id,parent_version,component_version,event_type,model_version) VALUES($1,$2,$3,'TradDraft',$4) ON CONFLICT DO NOTHING",[s.sid,parent,component,expectedModel]);
