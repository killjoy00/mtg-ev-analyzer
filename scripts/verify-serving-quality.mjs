...
for(const environment of ['mixed','powered-cube'])for(let i=0;i<20;i++){
 const run=await selectDatabaseRun(query,version,`floor-quality-${environment}-${i}`,environment,{daily:true,day});
...
const result={passed:true,serving_policy_version:SERVING_POLICY_VERSION,minimum_implied_trophy_score:20,day,daily_runs_checked:40,decisions_checked:decisions,custom_sets_checked:custom.length,historical_reads_and_trophy_score_preserved:true,inventory};
