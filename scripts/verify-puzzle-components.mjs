...
export async function verifyComponents(query,prepared,status=null) {
 for(const s of prepared.filter(s=>s.health.ready)) {
  ...
  const row=(await query(`SELECT c.status,v.manifest FROM corpus_components c JOIN corpus_set_versions v ON v.set_id=c.set_id AND v.corpus_version=c.component_version WHERE c.set_id=$1 AND c.component_version=$2 AND c.parent_version=$3`,[s.sid,v,parent])).rows[0];
  assert.ok(row,s.sid+': component missing');if(status)assert.equal(row.status,status);
  ...
  console.log(JSON.stringify({set:s.sid,component:v,status:row.status,puzzles:Number(n.n),usable:Number(n.usable),verified:true}));
 }
}
