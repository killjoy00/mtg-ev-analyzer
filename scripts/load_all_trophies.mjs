...
async function loadSet(s) {
  ...
  if(!remote){
    let ledgerBatch=[];
    const save=async()=>{if(!ledgerBatch.length)return;await query(`INSERT INTO corpus_trophy_trajectories(set_id,corpus_version,source_snapshot_id,source_draft_hash,event_type,wins,losses,qualified,included,puzzle_count,exclusion_reason) SELECT $1,$2,$3,x.* FROM jsonb_to_recordset($4::jsonb) AS x(source_draft_hash text,event_type text,wins smallint,losses smallint,qualified boolean,included boolean,puzzle_count integer,exclusion_reason text) ON CONFLICT(set_id,corpus_version,source_draft_hash) DO UPDATE SET source_snapshot_id=EXCLUDED.source_snapshot_id,losses=EXCLUDED.losses,qualified=EXCLUDED.qualified,included=EXCLUDED.included,puzzle_count=EXCLUDED.puzzle_count,exclusion_reason=EXCLUDED.exclusion_reason`,[s.id,DRAFT_RUN_CORPUS_VERSION,s.source_snapshot_id,JSON.stringify(ledgerBatch)]);ledgerBatch=[];};
