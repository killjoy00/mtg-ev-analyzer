...
// Direct reviewed connections only: the remote /v1/trophy-import load path is retired.
...
async function loadSet(s) {
  ...
  const existing=(await query('SELECT corpus_version FROM corpus_set_versions WHERE set_id=$1 AND corpus_version=$2',[s.id,DRAFT_RUN_CORPUS_VERSION])).rows[0];
  ...
  const actual=(await query('SELECT count(*)::int puzzles FROM draft_run_verified_puzzles WHERE set_id=$1 AND corpus_version=$2 AND source_snapshot_id=$3',[s.id,DRAFT_RUN_CORPUS_VERSION,s.source_snapshot_id])).rows[0];
