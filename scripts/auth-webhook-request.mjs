...
export function parseAuthWebhookRequest(value) {
  ...
  if(!['idle','check-secret','deploy-qa','deploy-qa-fail','delete-qa'].includes(value.operation))throw Error('Invalid Auth webhook request.');
  if(typeof value.reason!=='string'||!value.reason.trim())throw Error('Invalid Auth webhook request.');
  const needsCommit=['deploy-qa','deploy-qa-fail'].includes(value.operation);
