...
export const FROZEN_PREMIER_AUDIT_CORPUS_VERSION='elite-trophy-colour-stage-v7';
...
export function validateAudit(report) {
 ...
 for(const s of report.sets) {
  if(s.corpus_version!==FROZEN_PREMIER_AUDIT_CORPUS_VERSION||!/^[-a-z0-9]+$/.test(s.set)||!Array.isArray(s.blocked_sources)||s.approved_sources+s.blocked_sources.length!==s.included_sources||!/^[a-f0-9]{64}$/.test(s.input_signature)||!/^[a-f0-9]{64}$/.test(s.source_archive?.sha256))throw Error('Invalid audit accounting');
...
export async function applyAudit(query,report,auditHash) {
 ...
 // Preflight every audited v7 set before the first insert. The report cannot
 // point at a newer source archive or be transferred onto a later corpus.
 for(const s of report.sets) {
  const auditVersion=s.corpus_version;
  const found=(await query("SELECT manifest->'full_import' AS manifest FROM corpus_set_versions WHERE set_id=$1 AND corpus_version=$2",[s.set,auditVersion])).rows[0];
 ...
 for(const s of report.sets)if(s.blocked_sources.length)await query(`INSERT INTO corpus_source_exclusions(set_id,corpus_version,source_draft_hash,reason,evidence)
  ...
  ON CONFLICT(set_id,corpus_version,source_draft_hash) DO NOTHING`,[s.set,s.corpus_version,JSON.stringify(s.blocked_sources),auditHash,s.source_archive.sha256]);
