...
async function loadSet({set,rows}) {
  ...
  for(let i=0;i<rows.length;i+=250) {
    ...
    await query(`INSERT INTO draft_run_verified_puzzles(puzzle_id,set_id,source_draft_hash,corpus_version,pick_number,candidate_count,consensus_top_gap,support_entropy,interesting,payload) SELECT * FROM jsonb_to_recordset($1::jsonb) AS p(puzzle_id text,set_id text,source_draft_hash text,corpus_version text,pick_number smallint,candidate_count smallint,consensus_top_gap real,support_entropy real,interesting boolean,payload jsonb) ON CONFLICT(puzzle_id) DO NOTHING`,[JSON.stringify(batch)]);
  }
