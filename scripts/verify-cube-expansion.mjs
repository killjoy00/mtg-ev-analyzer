...
const check=run=>{assert.equal(run.length,8);assert.equal(new Set(run.map(p=>p.source_draft_hash)).size,8);assert.deepEqual(run.map(p=>p.pick_number),[2,3,4,5,6,7,8,9]);assert.ok(run.slice(6).every(p=>p.corpus_version===parent));};
...
const evidence={passed:true,runs:10000,sql_runs:20,traditional_decisions_selected:traditional,sql_traditional_decisions_selected:sqlTraditional,traditional_by_pick:byPick,model_changed:false};
